    ...
    return (
        <Card className={`card ${props.constraint.semver.strict === true && 'border-danger'} ${props.constraint.semver.tilde === true && props.constraint.semver.major && props.constraint.semver.minor && !props.constraint.semver.patch && 'border-warning'}`}>
            ...
            {props.constraint.semver.wildcard === true && props.constraint.semver.major === '*' ? (
                ...
                <h6 className="card-subtitle mb-3 text-muted">Constraint will be satisfied by versions matching <code>{props.constraint.semver.raw}</code>.</h6>
            )}

            {props.constraint.semver.caret === true && (
                <ExplainConstraintCaret constraint={props.constraint}/>
            )}

            {props.constraint.semver.tilde === true && (
                <ExplainConstraintTilde constraint={props.constraint}/>
            )}
            ...
            {props.constraint.semver.strict === true && (
                <ExplainConstraintStrict constraint={props.constraint}/>
            )}

            {props.constraint.semver.hyphen === true && (
                <ExplainConstraintHyphen constraint={props.constraint}/>
            )}

            {props.constraint.semver.wildcard === true && (
                <ExplainConstraintWildcard constraint={props.constraint}/>
            )}

            {props.constraint.semver.range === true && (
                <ExplainConstraintRange constraint={props.constraint} />
            )}
        </Card>
    );
};
